...
impl BurstMorphing {
    ...
    pub fn morph_burst(&mut self, cells: Vec<Cell>) -> Vec<Cell> {
        ...
        let target_pattern = self
            .target_patterns
            .iter()
            .min_by_key(|(size, _)| (*size as isize - buffer_len as isize).abs())
...
mod tests {
    ...
    fn test_burst_morphing_small_input_pads_to_min_size() {
        ...
        assert!(
            output.len() >= 2,
            "output must contain at least the real cells"
        );
        // Nearest target is 10 — verify padding cells have random non-zero payloads
        let padding_cells: Vec<_> = output
            .iter()
            .filter(|c| c.command == CellCommand::Padding)
            .collect();
    ...
    fn test_traffic_shaper_constant_rate_output() {
        ...
        rt.block_on(async {
            ...
            assert!(
                !output.is_empty(),
                "shape_cells must produce at least one cell"
            );
        });
    }
